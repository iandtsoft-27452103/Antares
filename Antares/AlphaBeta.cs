...
namespace Antares
{
    ...
    public class AlphaBeta
    {
        ...
        public static int futility_margin = 768; // Futility Pruningのマージン値 768
        public static int[] limit_table = new int[Ply_Max + 1];
        public static int[] futility_table = new int[Ply_Max + 1];
        public static int[] king_futility_table = new int[Ply_Max + 1];
        ...
        public struct AlphaBetaTree
        {
            ...
            public int pv_length2;
            public uint size_tt;
            //Sort sort;
            public Sort.MoveAndScore[,] mas_before;
            public Sort.MoveAndScore[,] mas_after;
            public Sort.MoveAndScore[,] mas_quies_before;
            public Sort.MoveAndScore[,] mas_quies_after;
            ...
            public int base_futility_margin = 0;
            //public ValueTT vtt;
            ...
            public AlphaBetaTree()
            {
                ...
                pv = new List<Move>();
                pv_length = 0;
                pv2 = new Move[Ply_Max];
                pv_length2 = 0;
                size_tt = 0x10000000; // 256MB
                mas_before = new Sort.MoveAndScore[Ply_Max, Moves_Max];
                mas_after = new Sort.MoveAndScore[Ply_Max, Moves_Max];
                mas_quies_before = new Sort.MoveAndScore[Ply_Max, CapMoves_Max];
                mas_quies_after = new Sort.MoveAndScore[Ply_Max, CapMoves_Max];
                iteration_max = 0;
                root_move_num = 0;
        ...
        public static void InitLimitTable()
        {
            for (int i = 0; i < Ply_Max; i++)
            {
                ...
                if (i <= 32)
                {
                    futility_table[i] = 300;
                    king_futility_table[i] = 600;
                }
                else if (i > 32 && i <= 48)
                {
                    futility_table[i] = 400;
                    king_futility_table[i] = 700;
                }
                else if (i > 48 && i <= 64)
                {
                    futility_table[i] = 430;
                    king_futility_table[i] = 730;
                }
                else if (i > 64 && i <= 80)
                {
                    futility_table[i] = 470;
                    king_futility_table[i] = 770;
                }
                else if (i > 80 && i <= 96)
                {
                    futility_table[i] = 500;
                    king_futility_table[i] = 830;
                }
                else if (i > 96 && i <= 106)
                {
                    futility_table[i] = 530;
                    king_futility_table[i] = 880;
                }
                else if (i > 106)
                {
                    futility_table[i] = 550;
                    king_futility_table[i] = 950;
                }
            }
        ...
        public static void TlpSearchWrapper(ref AlphaBetaTree abt, int color, int alpha, int beta, int depth, int ply, uint state_node, int prev_value, ref int return_value, ref List<Move> local_pv)
        {
            return_value = -Search(ref abt, color, alpha, beta, depth, ply, state_node, prev_value, ref local_pv);
        }

        public static int Search(ref AlphaBetaTree abt, int color, int alpha, int beta, int depth, int ply, uint state_node, int prev_value, ref List<Move> temp_pv)
        {
            int value, best_value, i, j, iret, temp_value, move_count, ifrom, ito, icap_pc, alpha_old, thread_index, prev_pv_length;
            uint state_node_new;
            Direction idirec;
            abt.moves[ply].Clear();
            ...
            // ※評価関数は直前のノードで呼んでいるので、ここでは呼び出さない。
            // ※静止探索も直前のノードで呼んでいるので、ここでは呼び出さない。
            ...
            if (abt.current_move[ply - 1].Value == (1 << 23))
            {
                temp_value = -abt.bt.EvalArray[ply - 1];
            }
            else
            {
            ...
            if (!abt.is_check[ply - 1] && 2 * Ply_Inc <= depth && (state_node & (uint)AlphaBetaTree.NodeState.node_do_null_move) != 0 && beta <= temp_value)
            {
                ...
                abt.bt.EvalArray[ply] = -temp_value;
                //abt.prev_eval[ply] = -temp_value;
                DoNull(ref abt.bt);
                // 深さ1だと浅すぎるかもしれない。
                int null_value = -Search(ref abt, color ^ 1, -beta, 1 - beta, 2 * Ply_Inc, ply + 1, state_node_new, -prev_value, ref local_pv);
                UnDoNull(ref abt.bt);
                m = new Move();
                abt.current_move[ply] = m;
                if (beta <= null_value)
            ...
            // ※これを入れる場合はライセンスがGPL3になる。=> 入れていないのでライセンスはMITである。
            // 【参考】
            ...
            {
                if (moves[i].CapPiece == Piece.King)
                {

                    Sort.MoveAndScore temp_mas0 = new Sort.MoveAndScore();
                    temp_mas0.move = moves[i];
                    temp_mas0.score = Value_Max;
                    abt.mas_before[ply, i] = temp_mas0;
                    continue;
                }
                ...
                temp_mas.score = v;
                abt.mas_before[ply, i] = temp_mas;
                UnDo(ref abt.bt, moves[i], color);
            }

            if (move_count > 1)
            {
                Sort.MergeSort(ref abt.mas_before, ref abt.mas_after, 0, move_count, ply);
            }
            else
            {
                abt.mas_after[ply, 0] = abt.mas_before[ply, 0];
            }

            int limit;


            //limit = (int)(limit_table[ply] * 2.5);
            //limit = limit_table[abt.bt.ply];
            //limit = (int)(move_count * 0.5);
            limit = move_count;

            int score_first = -abt.mas_after[ply, 0].score;
            int score_last = -abt.mas_after[ply, move_count - 1].score;
            bool is_stable = true;
            ...
            // score_firstとscore_lastの差が大きい場合は、局面が安定
            // score_firstとscore_lastの差が小さい場合は、局面が不安定
            ...
            for (i = 0; i < move_count; i++)
            {
                ...
                Move current_move = abt.mas_after[ply, i].move;
                abt.current_move[ply] = current_move;

                int current_score = -abt.mas_after[ply, i].score;

                if (current_move.PieceType == Piece.King)
                {
                    //futility_margin = king_futility_table[abt.bt.ply];//大槻将棋は600   1024
                    futility_margin = 600;
                }
                else
                {
                    //futility_margin = futility_table[abt.bt.ply];//大槻将棋は300    768
                    futility_margin = 300;
                }
                ...
                abt.bt.EvalArray[ply] = current_score;
                ...
                if (IsAttacked(abt.bt, abt.bt.SQ_King[color ^ 1], color ^ 1) > 0)
                {
                    ...
                    extension = Ply_Inc / 4; // 王手の時は0.25手延長する。
                    //Console.WriteLine("Check in " + ply.ToString());
                }
                ...
                if ((state_node & (uint)AlphaBetaTree.NodeState.node_pv) > 0)
                {
                    ...
                    if ((state_node & (uint)AlphaBetaTree.NodeState.node_slave) > 0)
                    {
                        state_node_new |= (uint)AlphaBetaTree.NodeState.node_slave;
                    }
                    //if (i == 0)
                    //extension += Ply_Inc / 2; // PVノードの最初の手は0.5手延長する。

                    //extension += ext_table[i];
                    if (i == 0)
                        //extension += 6;// 0.5手延長
                        extension += Ply_Inc / 2;
                    if (i == 1)
                    ...
                    if (i == 2)
                        extension += Ply_Inc / 8;
                    if (is_stable && i >= 12)//16
                        extension -= Ply_Inc / 2;
                    if (!is_stable && i >= 16)//22
                        extension -= Ply_Inc / 2;
                }
                else
                {
                ...
                int prev_ito = prev_move.To;
                if (ito == prev_ito && extension <= 0)
                {
                    // 前の手とTo位置が同じ場合は、0.25手延長する。
                    extension += Ply_Inc / 4;
                }

                //if (!is_stable && extension <= 0)
                //{
                // 局面が不安定な場合は、0.125手延長する。
                //extension += Ply_Inc / 8;
                //}
                ...
                /*if (current_move.PieceType == Piece.King)
                {
                    // 玉を動かす手はパラメータが大きく変動するので、0.25手延長する。
                    extension += Ply_Inc / 4;
                }*/

                // 延長は最大で0.75手までに制限する。
                if (extension > (Ply_Inc / 2))
                    extension = Ply_Inc / 2;

                //extension = 0;

                //extension = Ply_Inc / 2;
                //Console.WriteLine("rest_thread_num=" + rest_thread_num.ToString());

                // ※ 同一ノードでのマルチタスク探索は1回だけにした方が効率が良いと思われる。
                if (i > 0 && (state_node & (uint)AlphaBetaTree.NodeState.node_pv) > 0 && (state_node & (uint)AlphaBetaTree.NodeState.node_learning) == 0 && (state_node & (uint)AlphaBetaTree.NodeState.node_slave) == 0 && rest_thread_num > 0 && depth >= 3 * Ply_Inc && tlp_flag == false)
                {
                    // マルチスレッドで探索する。
                    rest_thread_num--;
                    tlp_flag = true;
                    ...
                    for (int k = 0; k <= l; k++)
                    {
                        tlp_abt[thread_index].current_move[k] = abt.current_move[k];
                        tlp_abt[thread_index].is_check[k] = abt.is_check[k];
                        tlp_abt[thread_index].bt.EvalArray[k] = abt.bt.EvalArray[k];
                    }
                    tlp_tpv.Clear();
                    /*for (int k = 0; k < tpv.Count; k++)
                    {
                        tlp_tpv.Add(tpv[k]);
                    }*/
                    //tlp_abt[thread_index].current_move = (Move[])abt.current_move.CopyTo()
                    //tlp_abt[thread_index].is_check = (bool[])abt.is_check.Clone();
                    //task = Task.Run(() => Search(ref tlp_abt[thread_index], color ^ 1, -beta, -alpha, depth - Ply_Inc + extension, ply + 1, state_node_new, -tlp_abt[thread_index].mas_after[ply, i].score));
                    tasks[thread_index] = Task.Run(() => TlpSearchWrapper(ref tlp_abt[thread_index], color ^ 1, -beta, -alpha, depth - Ply_Inc + extension, ply + 1, (state_node_new | (uint)AlphaBetaTree.NodeState.node_slave), -tlp_abt[thread_index].mas_after[ply, i].score, ref return_value, ref tlp_tpv));
                    UnDo(ref abt.bt, current_move, color);
                    continue;
                }

                // 残り深さが1以上ならば、次のノードを展開する。
                if (depth - Ply_Inc + extension >= Ply_Inc)
                {
                    tpv.Clear();// 多分これが必要
                    value = -Search(ref abt, color ^ 1, -beta, -alpha, depth - Ply_Inc + extension, ply + 1, state_node_new, current_score, ref tpv);// ※残り深さとstate_nodeは後で変更する。
                }
                else
                {
                    if (!abt.is_check[ply] && abt.current_move[ply].CapPiece != Piece.Empty)
                    {
                        // ※静止探索はまだ修正していない。
                        tpv.Clear();// 多分これが必要
                        value = -QuiesSearch(ref abt, color ^ 1, -beta, -alpha, ply + 1, 1, current_score, ref tpv);
                    }
                    else
                    {
                        // 評価値は計算済み
                        tpv.Clear();
                        value = current_score;
                        abt.tt.Store(abt.bt.CurrentHash, value, (Common.Color)(color), abt.is_check[ply], abt.current_move[ply], ply);// 最後のパラメータはply + 1かもしれない。
                    }
                }
                ...
                {
                    best_value = value;
                    abt.pv2[ply] = current_move;
                }
                else if (i > 0 && value > best_value/* && ply > abt.pv_length2*/)
                {
                    best_value = value;
                    abt.pv2[ply] = current_move;
                }
                ...
                if (alpha != alpha_old)
                {
                    ...
                    else
                    {
                        if (ply != 1)
                        {
                            if ((temp_pv.Count + tpv.Count) >= prev_pv_length || value == Value_Mate)
                            {
                                best_value = value;
                                local_pv.Clear();
                                for (j = 0; j < tpv.Count; j++)
        ...
        public static int QuiesSearch(ref AlphaBetaTree abt, int color, int alpha, int beta, int ply, int qui_ply, int prev_value, ref List<Move> temp_pv)
        {
            int value, best_value, alpha_old, stand_pat, move_count, i, j, prev_pv_length;
            ...
            for (i = 0; i < move_count; i++)
            {
                ...
                if (current_move.PieceType == Piece.King)
                {
                    futility_margin = 600;//大槻将棋は600   950
                }
                else
                {
                    futility_margin = 300;//大槻将棋は300    550
                }
                ...
                if (current_move.CapPiece == Piece.King)
                    continue;
                ...
                value = -QuiesSearch(ref abt, color ^ 1, -beta, -alpha, ply + 1, qui_ply + 1, -abt.mas_quies_after[ply, i].score, ref tpv);
                ...
                if (alpha != alpha_old)
                {
                    ...
                    else
                    {
                        if (ply != 1)
                        {
                            if ((temp_pv.Count + tpv.Count) >= prev_pv_length)
                            {
                                best_value = value;
                                local_pv.Clear();
                                for (j = 0; j < tpv.Count; j++)
