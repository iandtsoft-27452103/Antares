...
namespace Antares
{
    ...
    internal class MCTS2
    {
        ...
        public List<Move> TotalParam(int num_tasks, ref List<float> f, ref List<int> t, ref List<MCTS2> mcts_tree)
        {
            ...
            for (int i = 0; i < BTree.RootMoves.Length; i++)
            {
                ...
                {
                    win_rate_array.Add((float)(mcts_tree[0].NodeList[i + 1].WinRateSum / (float)(trial_count_array[i])));
                }
            }
        ...
        private void ExpandNode(int color, int parent_index, int ply)
        {
            ...
            //if (!tt.PolicyMoves.ContainsKey(BTree.CurrentHash))
            {
                ...
                for (int i = 0; i < moves.Count; i++)
                {
                    ...
                    try
                    {
                        v = -EvalWrapper(bt, color, bt.ply, m, false);
                        n.Value = v;
                    }
                    catch { v = Value_Min; }
